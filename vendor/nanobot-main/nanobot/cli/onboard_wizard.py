...
def run_onboard(initial_config: Config | None = None) -> OnboardResult:
    """
    在终端里循环显示一个主菜单，让用户反复进入不同配置页修改 config，最后选择保存或丢弃
    """
    # 确保 questionary 已经初始化
    _get_questionary()

    # 如果提供了初始配置，直接使用它；否则找默认配置，有则直接读，没有就创建新的Config()
    if initial_config is not None:
        # model_copy是深拷贝防止改变原对象
    ...
    while True:
        ...
        # Ctrl+C 不直接崩溃，而是当成“想退出”的一种信号，交给下面的退出逻辑处理。
        except KeyboardInterrupt:
            answer = None

        if answer is None:
            # 如果用户没回答或中段退出等，比较是否有改动，提示是否保存或丢弃
